...
function mapVisualisasi(raw: Record<string, string>[]) {
  return raw.map((r) => {
    try {
      // Handle headers with descriptive text in parentheses
      const tipeKey = Object.keys(r).find(k => k.toLowerCase().startsWith('tipe')) || 'tipe';
      const urlKey = Object.keys(r).find(k => k.toLowerCase().startsWith('url')) || 'url';
      const kategoriKey = Object.keys(r).find(k => k.toLowerCase().startsWith('kategori')) || 'kategori';
      
      const urutan = parseInt(r['urutan'] || r['Urutan'] || '0');
      
      return {
        id: String(r['id'] || r['Id'] || r['ID'] || '').trim(),
        tipe: String(r[tipeKey] || '').trim(),
        judul: String(r['judul'] || r['Judul'] || '').trim(),
        deskripsi: String(r['deskripsi'] || r['Deskripsi'] || '').trim(),
        url: String(r[urlKey] || '').trim(),
        kategori: String(r[kategoriKey] || r['Kategori'] || '').trim(),
        urutan: isNaN(urutan) ? 0 : urutan,
      };
    } catch (e) {
      console.error('Error mapping visualisasi row:', e, r);
      return {
        id: '',
        tipe: '',
        judul: '',
...
serve(async (req) => {
  ...
  try {
    ...
    // Safely stringify result with comprehensive error handling
    let responseBody: string;
    try {
      // Ensure all values in result are JSON-safe
      const cleanResult: Record<string, any> = {};
      for (const [key, value] of Object.entries(result)) {
        if (Array.isArray(value)) {
          cleanResult[key] = value.filter(item => {
            try {
              JSON.stringify(item);
              return true;
            } catch {
              console.error(`Skipping non-serializable item in ${key}`);
              return false;
            }
          });
        } else {
      ...
      responseBody = JSON.stringify(cleanResult);
    ...
    try {
      ...
      // Fallback if even error response fails to stringify
      return new Response(
