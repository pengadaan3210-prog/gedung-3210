    ...
    <div className="p-6 space-y-6">
      ...
      <Card className="shadow-md border-border">
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold text-foreground">Rekap Progres per Tahapan</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card className="shadow-md border-border">
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold text-foreground">Rekap per Penyedia / PIC</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="rounded-lg border border-border overflow-hidden">
            <Table>
              ...
              <TableBody>
                {Object.entries(
                  data.reduce((acc, d) => {
                    if (!acc[d.pic]) acc[d.pic] = { peran: d.peranPenyedia, items: [] };
                    acc[d.pic].items.push(d);
                    return acc;
                  }, {} as Record<string, { peran: string; items: typeof data }>)
                ).map(([pic, info]) => (
                  <TableRow key={pic}>
                    <TableCell className="font-medium">{pic}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">{info.peran}</TableCell>
